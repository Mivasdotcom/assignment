...
export const createStreaming = async (req, res) => {
    ...
    try {
      ...
      if (value.thumbnailUrl && !/^https?:\/\/.+\.(jpg|jpeg|png|webp)$/i.test(value.thumbnailUrl)) {
        return res.status(400).json({ error: "Thumbnail URL must be a valid image link" });
      }
...
export const updateStreaming = async (req, res) => {
    ...
    try {
      ...
      if (value.thumbnailUrl) {
        if (!/^https?:\/\/.+\.(jpg|jpeg|png|webp)$/i.test(value.thumbnailUrl)) {
          return res.status(400).json({ error: "Thumbnail URL must be a valid image link" });
        }
      }
...
export const playStreaming = async (req, res) => {
    const { id } = req.params;
    const streaming = await prisma.streaming.findUnique({ where: { id: parseInt(id) } });
    ...
    // Simple redirect
    return res.redirect(streaming.videoUrl);
